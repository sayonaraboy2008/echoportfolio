...
export const TerminalSection = ({ onOpenAdmin }) => {
  ...
  return (
    <section id="terminal" className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto relative z-10">
      <SectionHeading
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 mb-5">
        <span className="text-xs font-mono text-slate-500 dark:text-slate-400 mr-1 select-none">
          Quick Shortcuts:
        </span>
        {[
          ...
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl text-xs font-mono bg-white dark:bg-[#0c1017] border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:text-accent-cyan dark:hover:text-accent-cyan hover:border-accent-cyan/50 hover:bg-slate-100 dark:hover:bg-[#141a24] transition-all hover:-translate-y-0.5 active:scale-95 cursor-pointer shadow-sm flex items-center gap-1.5"
            title={`Run $ ${item.cmd}`}
          >
      ...
      <div
        className="bg-[#090d16] border border-slate-800 rounded-2xl shadow-2xl overflow-hidden cursor-text"
        onClick={() => inputRef.current?.focus()}
      >
        {/* Terminal Header */}
        <div className="bg-[#131926] px-4 py-3 border-b border-slate-800 flex items-center justify-between">
        ...
        <div
          ref={terminalBodyRef}
          className="p-5 sm:p-6 font-mono text-xs sm:text-sm max-h-[380px] min-h-[240px] overflow-y-auto custom-scrollbar flex flex-col gap-2.5"
        >
          {history.map((item, idx) => (
            <div key={idx} className="leading-relaxed">
              {item.type === 'input' ? (
                <div className="flex items-center gap-2 text-slate-300">
                  <span className="text-accent-violet font-bold">{data.shortName?.toLowerCase() || 'barkamol'}@arch</span>
                  <span className="text-accent-cyan font-bold">:~$</span>
                  <span className="text-white font-semibold">{item.content}</span>
                </div>
              ) : item.type === 'password' ? (
                <div className="flex items-center gap-2 text-slate-300">
                  <span className="text-amber-400 font-bold">[sudo] password:</span>
                ...
                <div className="text-amber-400/90 whitespace-pre-wrap">{item.content}</div>
              ) : (
                <div className="text-slate-300 whitespace-pre-wrap bg-black/40 p-3 rounded-lg border border-slate-800/60">
                  {item.content}
                </div>
              )}
            </div>
          ...
          <div className="flex items-center gap-2 mt-1">
            {isAwaitingPassword ? (
              <span className="text-amber-400 font-bold shrink-0">[sudo] password for admin:</span>
            ) : (
              <>
                <span className="text-accent-violet font-bold shrink-0">{data.shortName?.toLowerCase() || 'barkamol'}@arch</span>
